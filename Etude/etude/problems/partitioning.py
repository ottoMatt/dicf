...
def create_a_FileSol_wit_a_TP_distribution_naiveShort(
        file_sol, perc, seuilMin=1):
    '''
    ...
    otherlines = [line for line in copy_lines if not isinstance(
        line, futils.Clause)]
    ...
    # on ne sort pas car on s'en fout, çaa sera fait après
    distributed_lines = clauses + otherlines
    distrib_name = '_TPnaiveshortdist_per' + \
        str(perc) + '_' + 'seuil' + str(seuilMin)
    filename = file_sol.filename + distrib_name
    return futils.FileSol(file_sol.path, filename=filename,
                          lines=distributed_lines), int(nbTP)


def create_dcf_Agent_distribution(
        file_sol,
        nbAgents,
        method='naive_eq',
        outpath=None):
    ...
    if method == 'naive_eq':
        ...
        outpath, outfilename = futils.generate_kmet_distribution_simple(
            nbAgents, file_sol.path, file_sol.filename, outfile_path=outpath, outfile_name=filename)
...
def naive_eq(file_sol, nbAgents):
    ...
    # nan ! trop de comprehension ist tue la comprehensio n ^^ (en fait
    # potentiel err car non exclusif)
    clauses = [line for line in copy_lines if isinstance(line, futils.Clause)]
    ...
    for indexAgent, c in enumerate(futils.chunks(clauses, sizeAgent)):
        if not indexAgent >= nbAgents:
            ...
            new_agent_line = futils.IndexedLine(
